...
async function listarReclamacoes() {
    ...
    const apiUrl = 'https://88b7c53b-f8c2-4a85-8211-2fd5dc7fc089-00-wqhmyve1zz6r.picard.replit.dev/getallreclama?email=' + email;
    ...
    if (!response.ok) {
        ...
        if (data.length === 0) {
            ...
            dadosmensagem.innerHTML = "Aqui Estão Todas as Reclamações feitas Nessa Conta!";
...
listarReclamacoes();
        ...
        async function excluir(id){
            try {
                await confirmarExclusao(); // Aguarda a confirmação
    ...
    try {
        ...
        window.location.href = `/reclame-aqui/usuario/versolicitacao.html?nome=${encodeURIComponent(nome)}&id=${id}`;
    } catch (error) {
        console.error("Erro ao redirecionar com o nome:", error.message);
    }
}